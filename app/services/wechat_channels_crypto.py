...
从 scripts/spike/wechat_keystream.py 搬入生产位置。算法逻辑逐字保留；
generate_keystream 按解析后的整数 key 做 LRU 缓存（同 key 恒定）。
"""
...
def _emit_keystream(seed: int) -> bytes:
    rng = _Isaac64(seed)
...
def _generate_keystream_cached(seed: int) -> bytes:
    return _emit_keystream(seed)


def generate_keystream(decode_key: int | str) -> bytes:
    """由 decode_key 生成恰好 KEYSTREAM_SIZE 字节密钥流。纯函数，按 key 做 LRU 缓存。"""
    return _generate_keystream_cached(_parse_decode_key(decode_key))
